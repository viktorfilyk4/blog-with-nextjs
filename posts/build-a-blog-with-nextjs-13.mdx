...
export default function Header() {
  return (
    <header>
      ...
      // If you want to add a Client Component here like a theme switcher, mark that // component with
      "use client" and leave the majority of the header as an RSC
    </header>
  )
}
```
...
<Note>You can click any of the files in a file tree to view the source code on GitHub.</Note>
...
<Note>
  If you think it's bad that we call `getPost` _and_ `getPosts()`, remember the we wrapped
  `getPosts` in `cache`. `getPost` just calls `getPosts`, so we're not making any unnecessary
  requests to the filesystem (or wherever you're getting your posts from).
</Note>
